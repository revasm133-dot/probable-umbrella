  ...
  return (
    <div className="p-8 max-w-5xl mx-auto" data-testid="references-page">
        ...
        <div className="bg-white rounded-lg border border-[#E2E5DE] p-6 mb-6" data-testid="reference-form">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input data-testid="ref-title-input" type="text" value={form.title}
            ...
            <div>
              ...
              <input data-testid="ref-authors-input" type="text" value={form.authors}
            ...
            <div>
              ...
              <input data-testid="ref-year-input" type="number" value={form.year}
                onChange={(e) => setForm({ ...form, year: parseInt(e.target.value) })}
                className="mt-1 w-full px-3 py-2 rounded-md border border-[#E2E5DE] text-sm focus:outline-none focus:ring-2 focus:ring-[#4A6B46]/20 focus:border-[#4A6B46]" />
            </div>
            <div>
              <label className="text-xs font-medium text-[#5C605A]">Jurnal *</label>
              <input data-testid="ref-journal-input" type="text" value={form.journal}
            ...
            <div>
              ...
              <input data-testid="ref-doi-input" type="text" value={form.doi}
            ...
            <div className="col-span-2">
              ...
              <textarea data-testid="ref-abstract-input" value={form.abstract}
                onChange={(e) => setForm({ ...form, abstract: e.target.value })}
                rows={3} className="mt-1 w-full px-3 py-2 rounded-md border border-[#E2E5DE] text-sm focus:outline-none focus:ring-2 focus:ring-[#4A6B46]/20 focus:border-[#4A6B46] resize-none"
                placeholder="Ringkasan abstrak" />
            </div>
            <div>
              <label className="text-xs font-medium text-[#5C605A]">Kata Kunci</label>
              <input data-testid="ref-keywords-input" type="text" value={form.keywords}
            ...
            <div>
              ...
              <input data-testid="ref-notes-input" type="text" value={form.notes}
          ...
          <div className="flex gap-3 mt-4">
            <button data-testid="save-reference-button" onClick={addReference}
      ...
      <div className="relative mb-4">
        ...
        <input data-testid="reference-search" type="text" value={search}
      ...
      <div className="space-y-3" data-testid="references-list">
        {filtered.length === 0 ? (
          ...
          filtered.map((ref) => (
            <div key={ref.id} className="group p-4 rounded-lg border border-[#E2E5DE] bg-white hover:border-[#4A6B46]/20 transition-colors"
              data-testid={`reference-item-${ref.id}`}>
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <h4 className="text-sm font-medium text-[#1C1C19] leading-snug">{ref.title}</h4>
                  <p className="text-xs text-[#5C605A] mt-1">{ref.authors} ({ref.year}). <em>{ref.journal}</em></p>
                  {ref.doi && <p className="text-xs text-[#4A6B46] mt-1">DOI: {ref.doi}</p>}
                  {ref.keywords && (
                    <div className="flex gap-1 mt-2 flex-wrap">
                      {ref.keywords.split(",").map((kw, i) => (
                        <span key={i} className="text-[10px] px-2 py-0.5 rounded bg-[#F0F2ED] text-[#5C605A]">{kw.trim()}</span>
                      ))}
                    </div>
                  )}
                </div>
                <button data-testid={`delete-ref-${ref.id}`} onClick={() => deleteReference(ref.id)}
